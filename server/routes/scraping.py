...
from server.tasks.scraping import scrape_site_task, cleanup_scrape_job
...
def cleanup_job(job_id):
    ...
    try:
        ...
        if job.status not in ['completed', 'failed', 'cancelled']:
            return jsonify({'error': 'Job must be completed, failed, or cancelled before cleanup'}), 400
...
def get_scraping_stats():
    ...
    try:
        ...
        ).group_by(ScrapeJob.status).all()

        # Get total images scraped
        total_images = db.session.query(
            func.sum(ScrapeJob.images_scraped)
        ).scalar() or 0

        # Get recent jobs (last 7 days)
        from datetime import datetime, timedelta
        week_ago = datetime.utcnow() - timedelta(days=7)
        recent_jobs = ScrapeJob.query.filter(
