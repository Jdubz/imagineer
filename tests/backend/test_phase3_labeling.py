...
from server.services.labeling import (
    encode_image, get_labeling_prompts, label_image_with_claude, batch_label_images
)
from server.database import db, Image, Album, AlbumImage, Label
...
class TestBatchLabeling:
    ...
    def test_batch_label_images_success(self, mock_label, temp_output_dir):
        ...
        mock_label.side_effect = [
            ...
            {'status': 'success', 'description': 'Image 2', 'nsfw_rating': 'SAFE', 'tags': ['blue']},
...
class TestLabelingAPI:
    ...
    def test_label_album_endpoint_success(self, mock_batch, client):
        ...
        mock_batch.return_value = {
            ...
            'failed': 0,
            'results': [
                {'status': 'success', 'description': 'Image 1', 'nsfw_rating': 'SAFE', 'tags': ['test']},
                {'status': 'success', 'description': 'Image 2', 'nsfw_rating': 'SAFE', 'tags': ['test']}
            ]
        }
        ...
        with patch('server.auth.check_auth') as mock_auth:
            ...
            response = client.post(f'/api/labeling/album/{album_id}',
