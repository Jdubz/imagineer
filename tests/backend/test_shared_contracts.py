...
def _python_type_matches(schema_fragment: dict[str, Any], value: Any) -> bool:
    ...
    if isinstance(schema_type, list):
        return any(_python_type_matches({**schema_fragment, "type": sub}, value) for sub in schema_type)
...
def _validate_against_schema(payload: dict[str, Any], schema: dict[str, Any]) -> None:
    ...
    for key, fragment in properties.items():
        if key not in payload:
            continue
        assert _python_type_matches(fragment, payload[key]), f"Field '{key}' violates schema constraints"
...
def test_python_and_typescript_shared_types_are_in_sync() -> None:
    ...
    assert ts_contents == expected_ts, "TypeScript shared types are out of date. Run scripts/generate_shared_types.py"
    assert py_contents == expected_py, "Python shared types are out of date. Run scripts/generate_shared_types.py"
