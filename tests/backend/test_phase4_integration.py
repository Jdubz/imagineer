...
class TestScrapingWorkflowIntegration:
        ...
        with client.application.app_context():
            ...
            job = ScrapeJob(
                ...
                scrape_config=json.dumps(
                    {"depth": 5, "max_images": 10000, "follow_links": True, "download_images": True}
                ),
                status="running",
                progress=45,
                images_scraped=4500,
    ...
    def test_scraping_with_duplicate_images(self, client, temp_output_dir):
        ...
        with client.application.app_context():
            ...
            existing_image = Image(
                filename="duplicate.jpg", file_path="/tmp/duplicate.jpg", is_public=True
            )
            ...
            # Create job
            job = ScrapeJob(
                name="Duplicate Test Job", source_url="https://example.com", status="running"
            )
            db.session.add(job)
            db.session.commit()
            job_id = job.id
    ...
    def test_scraping_album_integration(self, client):
        ...
        with client.application.app_context():
            ...
            for i in range(3):
                image = Image(
                    filename=f"scraped_{i}.jpg", file_path=f"/tmp/scraped_{i}.jpg", is_public=True
                )
