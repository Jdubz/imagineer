...
class TestCompleteWorkflow:
    ...
    def test_complete_labeling_workflow(self, mock_label, mock_auth, client):
        ...
        with client.application.app_context():
            # Create image
            image = Image(filename="label_test.jpg", file_path="/tmp/label_test.jpg", is_public=True)
            db.session.add(image)
            db.session.commit()
            image_id = image.id
            
            # Label the image
            response = client.post(f'/api/labeling/image/{image_id}',
            ...
            caption_labels = [l for l in labels if l.label_type == 'caption']
            assert len(caption_labels) == 1
            assert 'beautiful blue square' in caption_labels[0].label_text
            
            tag_labels = [l for l in labels if l.label_type == 'tag']
            assert len(tag_labels) == 4
            tag_texts = [l.label_text for l in tag_labels]
    ...
    def test_batch_labeling_workflow(self, mock_batch, mock_auth, client):
        ...
        mock_batch.return_value = {
            ...
            'failed': 0,
            'results': [
                {'status': 'success', 'description': 'First image', 'nsfw_rating': 'SAFE', 'tags': ['first']},
                {'status': 'success', 'description': 'Second image', 'nsfw_rating': 'SAFE', 'tags': ['second']}
            ]
        }
        ...
        with client.application.app_context():
            ...
            response = client.post(f'/api/labeling/album/{album.id}',
...
class TestErrorHandling:
    ...
    def test_missing_file_handling(self, client):
        ...
        with client.application.app_context():
            ...
            image = Image(filename="missing.jpg", file_path="/nonexistent/path/missing.jpg", is_public=True)
            db.session.add(image)
            db.session.commit()
            image_id = image.id
...
class TestDataConsistency:
    ...
    def test_label_image_relationship_consistency(self, client):
        ...
        with client.application.app_context():
            ...
            # Create labels
            caption = Label(image_id=image.id, label_text="Test caption", 
                          label_type="caption", source_model="test")
            tag1 = Label(image_id=image.id, label_text="tag1", 
                        label_type="tag", source_model="test")
            tag2 = Label(image_id=image.id, label_text="tag2", 
                        label_type="tag", source_model="test")
            ...
            tag_labels = [l for l in image.labels if l.label_type == "tag"]
