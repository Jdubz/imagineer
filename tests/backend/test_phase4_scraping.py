...
from server.tasks.scraping import cleanup_scrape_job, import_scraped_images, scrape_site_task
    ...
    def test_scrape_site_task_success(self, mock_popen, client):
        ...
        with client.application.app_context():
            ...
            with patch("server.tasks.scraping.import_scraped_images") as mock_import:
                mock_import.return_value = {"imported": 50, "album_id": 1}
...
class TestScrapingAPI:
    ...
    def test_cleanup_job_success(self, mock_cleanup, client):
        ...
        with patch("server.auth.check_auth") as mock_auth:
            ...
            response = client.post(
                f"/api/scraping/jobs/{job_id}/cleanup",
                headers={"Authorization": "Bearer admin_token"},
            )
    ...
    def test_get_scraping_stats(self, client):
        ...
        with client.application.app_context():
            ...
            job2 = ScrapeJob(
                name="Job 2", source_url="https://example2.com", status="running", images_scraped=25
            )
            job3 = ScrapeJob(
                name="Job 3", source_url="https://example3.com", status="failed", images_scraped=0
            )
...
class TestScrapingIntegration:
    ...
    def test_scraping_error_handling(self, client):
        ...
        with patch("server.auth.check_auth") as mock_auth:
            ...
            response = client.post(
                "/api/scraping/start", json={}, headers={"Authorization": "Bearer admin_token"}
            )
    ...
    def test_scraping_pagination(self, client):
        ...
        with client.application.app_context():
            ...
            for i in range(25):
                job = ScrapeJob(
                    name=f"Job {i}", source_url=f"https://example{i}.com", status="completed"
                )
                jobs.append(job)
