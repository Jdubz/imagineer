...
def main():  # noqa: C901
    ...
    parser.add_argument(
        "--output", type=str, default=None, help="Output filename (auto-generated if not specified)"
    )
    ...
    parser.add_argument(
        ...
        nargs="+",
        default=None,
        help="Paths to LoRA weights files (.safetensors)",
    )
    parser.add_argument(
        "--lora-weights",
        type=float,
        nargs="+",
        default=None,
        help="LoRA weights/scales (one per LoRA)",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="(Deprecated) Single LoRA path, use --lora-paths",
    )
    parser.add_argument(
        "--lora-weight",
        type=float,
        default=0.5,
        help="(Deprecated) Single LoRA weight, use --lora-weights",
    )
    ...
    if args.output is None:
        ...
        prompt_slug = (
            "".join(c if c.isalnum() or c in (" ", "-", "_") else "" for c in args.prompt[:50])
            .strip()
            .replace(" ", "_")
        )
        ...
        for lora_path_str, weight in zip(lora_paths, lora_weights):
            ...
            if lora_path.exists():
                ...
                pipe.load_lora_weights(
                    lora_path.parent, weight_name=lora_path.name, adapter_name=lora_path.stem
                )
                loaded_loras.append(
                    {"path": str(lora_path), "name": lora_path.stem, "weight": weight}
                )
            else:
