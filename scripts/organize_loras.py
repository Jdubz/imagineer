...
def queue_preview_job(
    ...
    try:
        ...
        response = requests.post(f"{api_url}/api/generate", json=job_data, timeout=5)
...
def generate_preview(
    ...
    try:
        print(
            f"    {BLUE}Generating preview (steps={steps}, guidance={guidance_scale})...{RESET}",
            end="",
            flush=True,
        )
...
def organize_lora(
    lora_file, lora_base_dir, index, skip_preview=False, queue_previews=False, config=None
):
    """
    ...
    config_success = create_lora_config(
        lora_folder, dest_file, preview_filename if preview_success else None, default_weight=0.6
    )
...
def reconcile_index(lora_base_dir, index):
    ...
    actual_folders = {
        d.name
        for d in lora_base_dir.iterdir()
