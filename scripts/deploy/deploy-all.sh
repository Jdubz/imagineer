...
check_configuration() {
    ...
    if [ -f "$WEB_DIR/.env.production" ]; then
        ...
        if grep -q "VITE_API_BASE_URL=https://imagineer-api.joshwentworth.com/api" "$WEB_DIR/.env.production"; then
...
run_health_checks() {
    ...
    if curl -f https://imagineer-api.joshwentworth.com/api/health &> /dev/null 2>&1; then
        ...
        print_status "  Wait 1-2 minutes and try: curl https://imagineer-api.joshwentworth.com/api/health"
    fi
...
print_summary() {
    ...
    echo "URLs:"
    echo "  API:      https://imagineer-api.joshwentworth.com/api"
    ...
    echo "  Public:   curl https://imagineer-api.joshwentworth.com/api/health"
