        ...
        with safe_open(lora_path, framework="pt", device="cpu") as f:
            ...
            if te1_keys:
                return (
                    False,
                    "sdxl",
                    f"Detected SDXL dual text encoder ({len(te1_keys)} te1/te2 keys)",
                )
            ...
            for pattern in illustrious_patterns:
                ...
                if matching_keys:
                    ...
                    numbered_pattern = any(
                        k
                        for k in matching_keys
                        if ".1.transformer_blocks" in k or ".0.transformer_blocks" in k
                    )
                    if numbered_pattern:
                        return (
                            False,
                            "illustrious",
                            f"Detected Illustrious/SDXL architecture (found {pattern} pattern)",
                        )
...
Examples:
  ...
  python scripts/clean_loras.py --lora-dir /path/to/loras
    ...
    parser.add_argument(
        "--clean", action="store_true", help="Actually move incompatible files (default: dry run)"
    )

    parser.add_argument(
        "--no-move", action="store_true", help="Just report, don't move files even with --clean"
    )

    args = parser.parse_args()

    scan_lora_directory(args.lora_dir, dry_run=not args.clean, move_incompatible=not args.no_move)
